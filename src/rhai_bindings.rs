...
impl TraceSpan {
	fn start(ctx: Context, name: &str) -> Self {
		Self {
			state: Some(
				Arc::new(Mutex::new(TraceSpanState {
					ctx,
					name: name.to_string(),
					start: Instant::now(),
					stopped: false
...
fn walk_impl(
	...
	if has_strings {
		...
		if rewrite && kind_id_set.is_none() && !ctx_walk.test_group_enabled() {
			if let Some(kind_ids) = simple_kind_ids(
				ctx_walk,
				language,
				&lang,
				&query_list
			...
			for node_ref in nodes.iter() {
				...
				let doc_id: INT = if captured.contains(&key) {
					...
					if (node_ref.is_error() || node_ref.is_missing()) && ctx_walk.debug_enabled() {
						...
						rule.call_within_context(&ctx_call, (node_ref.clone(),)).map_err(Box::<EvalAltResult>::from)?
					}
				}
				else {
					if (node_ref.is_error() || node_ref.is_missing()) && ctx_walk.debug_enabled() {
...
pub fn register_primitives(engine: &mut Engine, ctx: Context) {
	...
	engine.register_fn(
		...
		move |node: Arc<NodeRef>, before: INT, after: INT, same_line: bool| -> RhaiResult<Range> {
			...
			Ok(ctx_range_around.range_around(
				&node,
				before,
				after,
				same_line
	...
	engine.register_fn("next_sibling", |node: &mut Arc<NodeRef>| -> Option<Arc<NodeRef>> {
		node.next_sibling()
	});
	engine.register_fn("prev_sibling", |node: &mut Arc<NodeRef>| -> Option<Arc<NodeRef>> {
		node.prev_sibling()
	});
	...
	engine.register_fn(
		...
		move |doc: INT, width: INT, indent_style: &str, tab_width: INT| -> RhaiResult<String> {
			...
			Ok(ctx_doc_render_indent.doc_render_with_indent(
				doc,
				width,
				style,
				tab_width
	...
	engine.register_fn("last_index_of", |s: &mut ImmutableString, needle: &str| -> INT {
